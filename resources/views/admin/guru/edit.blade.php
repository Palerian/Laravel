...
@section('title', 'Edit Data Guru — SMK Miyamasuzaka')
@section('heading', 'Sunting Biodata Guru')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm space-y-5">
        <div class="border-b border-slate-200 pb-3">
            <h2 class="text-base font-bold text-slate-900">Perbarui Biodata Guru: {{ $guru->nama }}</h2>
            <p class="text-xs text-slate-500 mt-0.5">NIP: {{ $guru->nip }} • Mapel: {{ $guru->mata_pelajaran }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.guru.update', $guru) }}" class="space-y-4">
            ...
            <div>
                <label for="nama" class="block text-xs font-semibold text-slate-700 mb-1">Nama Lengkap & Gelar</label>
                <input type="text" id="nama" name="nama" value="{{ old('nama', $guru->nama) }}" required class="w-full text-xs rounded border-slate-300 focus:border-purple-700 focus:ring-purple-700 py-2 px-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="nip" name="nip" value="{{ old('nip', $guru->nip) }}" required class="w-full text-xs rounded border-slate-300 focus:border-purple-700 focus:ring-purple-700 py-2 px-3 font-mono">
                ...
                <div>
                    ...
                    <input type="text" id="mata_pelajaran" name="mata_pelajaran" value="{{ old('mata_pelajaran', $guru->mata_pelajaran) }}" required class="w-full text-xs rounded border-slate-300 focus:border-purple-700 focus:ring-purple-700 py-2 px-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="no_telepon" name="no_telepon" value="{{ old('no_telepon', $guru->no_telepon) }}" class="w-full text-xs rounded border-slate-300 focus:border-purple-700 focus:ring-purple-700 py-2 px-3 font-mono">
                </div>
            </div>

            <div>
                <label for="alamat" class="block text-xs font-semibold text-slate-700 mb-1">Alamat Domisili</label>
                <textarea id="alamat" name="alamat" rows="2" class="w-full text-xs rounded border-slate-300 focus:border-purple-700 focus:ring-purple-700 py-2 px-3">{{ old('alamat', $guru->alamat) }}</textarea>
            </div>
