...
@section('title', 'Edit Nilai Siswi — SMK Miyamasuzaka')
@section('heading', 'Sunting Nilai Evaluasi')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm space-y-5">
        <div class="border-b border-slate-200 pb-3">
            <h2 class="text-base font-bold text-slate-900">Perbarui Nilai: {{ $nilai->siswa->nama ?? 'Siswi' }}</h2>
            <p class="text-xs text-slate-500 mt-0.5">Mapel: {{ $nilai->mapel->nama ?? 'Mapel' }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.nilai.update', $nilai) }}" class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="jenis_nilai" name="jenis_nilai" required class="w-full text-xs rounded border-slate-300 focus:border-purple-700 focus:ring-purple-700 py-2 px-3">
                        @foreach (['Tugas', 'UH', 'UTS', 'UAS', 'Praktik Kejuruan'] as $jenis)
                            <option value="{{ $jenis }}" {{ old('jenis_nilai', $nilai->jenis_nilai) === $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="nilai" name="nilai" value="{{ old('nilai', $nilai->nilai) }}" min="0" max="100" required class="w-full text-xs rounded border-slate-300 focus:border-purple-700 focus:ring-purple-700 py-2 px-3 font-mono">
